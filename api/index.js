...
app.use(
  cors({
    credentials: true,
    origin: ['https://prime-pakistan.vercel.app'], //'http://127.0.0.1:5173',
  })
);
...
app.get('/api/get-user-data', async (req, res) => {
  ...
  if (token) {
    try {
      ...
      if (username === 'testaccount') res.json(username);
    } catch (e) {
...
app.post('/api/delete-record', async (req, res) => {
  ...
  const ContractDco = await ContractModel.deleteOne({
    pnNo,
  });
  res.json(ContractDco);
});
...
app.get('/api/all-contracts', async (req, res) => {
...
app.get('/api/get-data', async (req, res) => {
  ...
  const ContractDoc = await ContractModel.find({ section: tableId }).lean();
